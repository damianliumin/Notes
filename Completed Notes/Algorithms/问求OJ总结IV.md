...
本题就是模拟题，其实没什么难度，但是一开始题面有误(题面内容本来也就很多，比较容易读错)。卡了很久，遇到的坑如下：
...
看一个小方块内的数字能否和大方块内某个区域匹配。助教用了哈希表，我想用一个碰撞率低一些的哈希函数，但是这样既难算又容易TLE。最后还是用二维前缀和，直接就过了。

### B 废水处理

用并查集。过程中遇到一个bug: union_set的时候，如果x和y已经在一个集合，则不必union了。

### C Hello, World!

结合Floyd算法的思想即可。一开始忘了是无向图。

### D 无限的可能-infinite

字典树用数组存放，速度直接起飞，动态内存分配太慢了。
...
用组合数学即可。过程中有一个东西没想清楚，拖了一个多小时。感觉需要提高一下自己的数学修养了。
